...
bibliography: 2026-04-27-escaping-the-femtosecond-prison-promise-and-limits-of-flashmd.bib
...
# Introduction
As Richard Feynman once said, everything can be understood in terms of the jigglings and wigglings of atoms<d-cite key="feynman2011feynman"></d-cite> . The computational microscope used to examine this behavior is called Molecular Dynamics (MD). MD enables us to simulate atoms according to Newton's laws and observe processes such as protein folding, battery charging, and material fracture—all at the atomic scale.
...
FlashMD<d-cite key="bigi2025flashmd"></d-cite> tackles this second bottleneck head-on. Rather than following the traditional MD loop—calculating forces and integrating numerically with small time steps—FlashMD predicts the system's evolution directly over significantly larger time intervals—typically one to two orders of magnitude beyond the limits of classical integrators. Across water, proteins, metals, and battery materials, it recovers the correct statistical physics at speedups of 8× to 64×.
...
The challenge lies in the sheer repetition required. As shown in the workflow below, every MD simulation is built around a core loop that must be executed millions to billions of times
...
{% include figure.liquid path="assets/img/2026-04-27-flashmd/MD_simulation_workflow.png" class="img-fluid" caption="Figure 1: The MD simulation loop. Force calculation (Step 2) and integration (Step 3) must be repeated millions to billions of times to generate a meaningful trajectory." %}
...
This raises a natural question: **which step consumes the most time?** Historically, the answer was force calculation—but as we'll see, even when forces are cheap, the timestep limit remains.
...
This algorithm is **symplectic**, meaning it preserves the geometric structure of phase space. This is crucial: symplectic integrators approximately conserve the Hamiltonian (total energy) even with finite $\Delta t$, preventing the systematic energy drift that would cause non-symplectic methods to fail over long simulations<d-cite key="hairer2006geometric"></d-cite>. But there's a catch: stability requires $\Delta t \sim 0.5\text{–}1$ femtoseconds<d-cite key="leach2001timestep"></d-cite>—small enough to resolve the fastest atomic vibrations (e.g., hydrogen bond stretches). Larger steps cause energy drift, and eventually, atoms fly apart in numerical explosions. This is the **femtosecond prison** we introduced earlier—the fundamental timestep barrier that limits all classical MD.

Every loop iteration requires:
- **Computing forces** via $\nabla V$ (expensive)
- **Taking a tiny timestep** (limiting)

The first bottleneck—force evaluation—has been largely solved by Machine Learning Interatomic Potentials. But before we get there, let's understand why it was a problem in the first place.
...
But what if we could learn the potential energy surface itself—bypassing both the quantum calculation and the classical approximation? Machine Learning Interatomic Potentials (MLIPs) changed this.
...
Recall that every timestep requires evaluating $\mathbf{F}_i = -\nabla_i V(\mathbf{Q})$ for every atom at every timestep. Historically, this forced a compromise: precise but computationally prohibitive quantum mechanics (scaling $\mathcal{O}(N^3)$ or worse), or fast but inaccurate classical force fields.

MLIPs resolve this by learning a surrogate map from atomic configurations to potential energy, reducing the complexity to $\mathcal{O}(N)$<d-cite key="mishin2021machine"></d-cite>.
...
1. **Physical Validity:** It guarantees a conservative force field ($\nabla \times \mathbf{F} = 0$), which is consistent with fundamental physics.
...
- **Symmetry:**  Architectures are designed to be $E(3)$—invariant-rotating or translating the molecule does not change the predicted energy $V_\theta$. This automatically ensures that predicted forces are $E(3)$-equivariant (they rotate correctly with the atoms). 

In practice, MLIPs are trained to minimize errors in both energies and forces simultaneously:
...
MLIPs successfully decoupled accuracy from computational cost. They allow us to run simulations with ab initio accuracy at near-classical speed—force evaluations that once took seconds now take milliseconds.
...
With MLIPs, we've solved the force bottleneck. But the femtosecond prison remains. The question is no longer how to compute forces faster—it's how to bypass the integrator entirely.

---

# FlashMD: Escaping the Femtosecond Prison
FlashMD<d-cite key="bigi2025flashmd"></d-cite> proposes a radical alternative: **bypass the runtime loop of force calculation and numerical integration**. Instead of learning a potential energy surface and feeding its gradients into a classical integrator, FlashMD learns the **dynamical map** itself—a neural network that directly predicts how the system evolves over a large time interval:
...
But first: how is FlashMD actually built?
...
Consider a water molecule. If we rotate the entire simulation box by 90°, the physics does not change. Energies remain identical, and force vectors rotate by exactly the same 90° to follow the atoms. Formally, for any rotation matrix $\mathcal{R}$, a physically valid model $\mathcal{F}$ (like FlashMD's $\mathcal{G}_\theta$) must satisfy:
...
|----------|----------|-----------|-----------|
| **Hard constraints** | NequIP<d-cite key="NequIP_Batzner2022"></d-cite>, MACE<d-cite key="MACE_ALLEGRO_leimeroth2025machine"></d-cite> | Built using irreducible representations (e.g., spherical harmonics) | Exact equivariance, but computationally expensive |
...
3. **Message update.** The Transformer outputs are processed through a feedforward network to produce updated bond messages $\mathbf{f}_{ij}^{(l+1)}$, which become the inputs for the next message-passing layer. After $L$ layers, the accumulated per-atom representations are passed through a feed-forward network to predict the target property (in FlashMD's case: displacements and momenta).

**Why this matters for FlashMD.** The unconstrained design gives PET a remarkable theoretical property: even a single message-passing layer is, in principle, a **universal approximator** with high body order and angular resolution<d-cite key="PET_pozdnyakov2023smooth"></d-cite>. This means PET can, in principle, represent arbitrarily complex atomic interactions—exactly the kind of expressivity needed to learn a dynamical map that replaces hundreds of Velocity Verlet steps.
...
Instead, the authors adopt a **statistical evaluation**: generate reference trajectories with a trusted conventional force field (PET-MAD<d-cite key="mazitov2025pet"></d-cite>, a standard MLIP trained to predict forces, not dynamics), run FlashMD under identical conditions, and compare **ensemble-averaged properties**—density, radial distribution functions, phase transition temperatures, and diffusion coefficients. If these statistical fingerprints match, the dynamics are physically meaningful, regardless of whether individual trajectories agree.
...
**Liquid Water.**
Despite its apparent simplicity, water is notoriously difficult to simulate due to its fluctuating hydrogen-bond network. At 450 K, the water-specific model reproduces radial distribution functions—the spatial arrangement of oxygen and hydrogen atoms—nearly perfectly. Temperature control via a Langevin thermostat maintains deviations under 1 K. Strides of up to **16 fs** are stable, representing a **64× speedup** over the 0.25 fs baseline typically required for rigid water models (see [Figure 4](#fig:water)).

{% include figure.liquid path="assets/img/2026-04-27-flashmd/water_experiment_flashmd.png" class="img-fluid" label="fig:water" caption="Figure 4: FlashMD reproduces the radial distribution functions of liquid water at 450 K, even at 16 fs strides (64× speedup). The model-predicted structure (blue) matches the ground truth (black) nearly perfectly.  Adapted from Bigi et al. (2025)"%}

**Solvated Alanine Dipeptide.**
This small peptide in water serves as a minimal proxy for protein flexibility. The critical benchmark is the **Ramachandran plot** ([Figure 5](#fig:universal) a), which maps the accessible backbone conformations. FlashMD recovers the correct distribution of dihedral angles even at strides **32× larger** than standard MD—strong evidence that it captures meaningful conformational dynamics, not merely static snapshots.

**Aluminum Surface Pre-melting.**
Metal surfaces exhibit subtle, layer-dependent dynamics at high temperatures: surface atoms become mobile before the bulk melts. FlashMD reproduces the characteristic **anisotropic softening pattern** (different vibration amplitudes along different crystal axes) and captures the formation and migration of dynamic surface defects—all at **64 fs strides** (64× speedup) ([Figure 5](#fig:universal) b).

**Lithium Thiophosphate: Superionic Transition.**
Perhaps the most striking result involves a solid-state battery electrolyte. At elevated temperatures, lithium ions become highly mobile in a "superionic" phase—a collective transition critical for battery performance. FlashMD predicts the transition temperature at 675 K (within the expected range) and reproduces the dramatic jump in lithium conductivity, achieving an **8× speedup** ([Figure 5](#fig:universal) c). Some systematic errors appear at extreme temperatures, but the qualitative physics is captured.

{% include figure.liquid path="assets/img/2026-04-27-flashmd/universal_experiments_flashmd.png" class="img-fluid" label="fig:universal" caption="Figure 5: Universal Experiments FlashMD. Adapted from Bigi et al. (2025)"%}
...
In the next section, we strip away the thermostats and run FlashMD in the most unforgiving setting: the microcanonical (NVE) ensemble, where energy must be conserved by the dynamics alone. This reveals fundamental limitations of the learned approach—and points toward what must be solved before learned dynamics can be trusted without external controls.

# An Exploratory Study on Failure Modes

To move beyond theoretical concerns, we conduct a systematic exploratory study on a concrete system: a periodic box of 258 TIP3P water molecules simulated with OpenMM as ground truth. We then trained FlashMD models under a variety of settings and asked a blunt question: can they conserve energy in NVE rollouts—where no thermostat can mask drift, and total energy must remain constant by the dynamics alone? This is the most unforgiving test of physical correctness.
...
**Evaluation.** Each trained model is rolled out for 50 ps in an NVE simulation, starting from a 300K equilibrated configuration.

**Model Configuration.** We used a 4-layer PET backbone with 128 hidden dimensions and a cutoff radius of 5 Å, matching the architecture described in the FlashMD paper. Models were trained for 400 epochs using AdamW with a learning rate of 1e-3 on a A100 GPU.

Before tackling the ablation studies, we first asked a profound question: do the trained models even behave sensibly?



{% include figure.liquid path="assets/img/2026-04-27-flashmd/comparison_detailed_analysis.png" class="img-fluid" label="fig:basecomparison" caption="Figure 6: Comparison of temperature and energy stability across various model setups against the OpenMM ground truth. All trained models exhibit systematic cooling and energy drift, despite appearing numerically stable. The ground truth (black) maintains constant energy; none of the models do." %}

The first results are shown in [Figure 6](#fig:basecomparison). Across different random seeds and different starting configurations, the picture is consistent. All trained models produce trajectories that look stable—no immediate explosions, no obvious numerical blow-ups.
...
To understand where this energy is going, we looked at the momentum distributions predicted by the models and compared them to ground truth. Since temperature is directly proportional to the mean kinetic energy—which depends on $$\langle p^2 \rangle$$—any systematic bias in momentum distributions immediately translates into incorrect thermodynamics.

{% include figure.liquid path="assets/img/2026-04-27-flashmd/velocity_momentum_species_grid.png" class="img-fluid" label="fig:base_mom_dist" caption="Figure 7: Species-resolved momentum and velocity distributions. Oxygen momenta are systematically underestimated (shifted left relative to ground truth), while hydrogen distributions are closer to correct. Since oxygen carries most of the kinetic energy, this bias explains the observed cooling." %}

The picture is surprisingly consistent across models ([Figure 7](#fig:base_mom_dist)). Hydrogen velocities look roughly reasonable, but oxygen momenta are systematically shifted toward lower values compared to ground truth. Since oxygen atoms carry most of the system’s kinetic energy, even a small bias here translates into a large temperature drop.
...
We also notice something interesting at the species level: when we crank the momentum weight up to $w_p = 10$, the oxygen velocity and momentum distributions move noticeably closer to the ground truth, while hydrogen barely changes (see [Figure 9](#fig:mom10)). That hints at a mass-dependent effect in how momentum errors show up. Still, the big picture doesn’t change — energy continues to drift, which reinforces that loss reweighting alone isn’t enough to recover physically consistent dynamics.

{% include figure.liquid path="assets/img/2026-04-27-flashmd/mom_10_velocity_momentum_species_grid.png" class="img-fluid" label="fig:mom10" caption="Figure 9: Species-wise velocity and momentum distributions under large momentum loss weighting ($w_p=10$). Oxygen improves; hydrogen does not." %}
...
{% include figure.liquid path="assets/img/2026-04-27-flashmd/mass_scaling_nve_comparison.png" class="img-fluid" label="fig:mass-scaling" caption="Figure 10: NVE trajectory comparison between standard MSE and mass-scaled MSE loss. Left column shows the initial drift phase (0–10 ps), right column the full 50 ps trajectory. Both models exhibit systematic cooling relative to the ground truth." %}
...
## The Takeaway
Whether we reweight momenta, mass-scale the loss, or tune coefficients, the result is the same: systematic energy drift. The problem isn't how much the model cares about momenta or how errors are weighted—it's structural.
...
# Summary
We have seen that FlashMD introduces a fundamentally new way to think about MD simulation: bypassing runtime force evaluation and numerical integration by directly learning the evolution of the system. In doing so, it addresses the two central bottlenecks of classical MD—the cost of force calculations and the femtosecond timestep stability limit—and demonstrates impressive speedups.

Our independent exploratory study demonstrates a clear limitation. When external controls such as thermostats are removed and the model is asked to operate in the microcanonical (NVE) ensemble, FlashMD does not conserve energy. This failure persists across loss reweighting and mass-scaled formulations, indicating a structural issue rather than a tuning problem.

This does not diminish FlashMD's contributions. It represents an impressive step toward longer-stride learned dynamics and demonstrates that statistical accuracy is achievable without explicit physical constraints. But it does make one thing clear: what is still missing is the mathematical guarantee that classical integrators provide.

The question we posed in the introduction—Can a model learn to respect the laws of physics without being explicitly constrained?—now has a nuanced answer. With thermostats and external controls, yes. In pure Hamiltonian dynamics, where energy must conserve itself—not yet.